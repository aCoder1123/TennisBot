...
public class DriveForDistance extends CommandBase {
    ...
    // private PIDController m_rotController = new PIDController(kTurnP, kTurnI, kTurnD);
  ...
  public void execute() {
    ...
    distanceDriven = Math.sqrt(Math.pow(m_swerve.getFieldPosition().getX() - currentPos.getX(), 2) + Math.pow(m_swerve.getFieldPosition().getY() - currentPos.getY(), 2));
    xSpeed = dControllerOn ? m_dController.calculate(distanceDriven) * Math.sin(Math.toRadians(driveAngle)) : m_xController.calculate(m_swerve.getFieldPosition().getX());
    ySpeed = dControllerOn ? m_dController.calculate(distanceDriven) * Math.cos(Math.toRadians(driveAngle)) :m_yController.calculate(m_swerve.getFieldPosition().getY());
